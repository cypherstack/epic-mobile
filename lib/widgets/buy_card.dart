...
class FiatBuyCard extends ConsumerWidget {
  const FiatBuyCard({
    Key? key,
    required this.transaction,
  }) : super(key: key);

  final GuardarianTransaction transaction;
  ...
  (String, Color) prettyStatus(String statusString, BuildContext context) {
    ...
    switch (status) {
      ...
      case ChangeNowTransactionStatus.Waiting:
        if (DateTime.now()
                .difference(DateTime.parse(transaction.createdAt!))
                .inMinutes >
            45) {
          return (
            "Expired",
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        Text(
                          "${transaction.estimateBreakdownFromAmount} ${transaction.fromCurrency}",
                          style: STextStyles.bodySmall(context),
                        ),
                        Text(
                          "${transaction.estimateBreakdownToAmount.toStringAsFixed(1)} ${transaction.toCurrency}",
                          style: STextStyles.bodySmallBold(context),
                        ),
                      ],
                    ),
                    Row(
                      ...
                      children: [
                        ...
                        Text(
                          Format.extractDateFrom(
                            transaction.createdAtTimestamp,
                          ),
                          style: STextStyles.bodySmall(context),
                        ),
                      ],
